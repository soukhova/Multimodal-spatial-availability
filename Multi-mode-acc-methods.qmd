...
workers <- OD_v_HW %>%
  group_by(VORIZT1259,
           mode) %>%
  summarize(workers = n()*ELE_G_POND_ESC2, #we should be summing the number of workers by the weight but we don't have a weight... so for now, 1 row = 1 trip. 
            .groups = "drop")%>%
...
jobs <- OD_v_HW %>%
  group_by(VDESZT1259) %>%
  summarize(jobs = n()*ELE_G_POND_ESC2,
            .groups = "drop") %>%
