...
# Illustrative example: operationalizing spatial availability for multiple modes
...
See @soukhovIntroducingSpatialAvailability2023 for details, but in short, the proportional allocation mechanism of $F^t_{ij}$ ensures that each $V_i$ value assigned to each $i$ sums to the total number of opportunities. This is important, because opportunities in a region are finite. If one person interacts with the opportunity at a given time, it is taken. In this way, if one person has the ability to reach more opportunities through a faster mode, for instance, than that person has more opportunities they can potentially interact with than other people. In a region with finite opportunities, one person being able to access more opportunities means there are fewer opportunities available for people using slower modes. 

can be formulated to consider  characteristics of different population groups. For instance, consider two sub-populations at population center $A$. One sub-population uses mode $x$ and the other uses mode $z$ to access jobs at employment center $1$. Mode $x$ is faster than mode $z$, so the sub-population that use $x$ can potentially access more jobs at employment center $1$ than the other sub-population as a result of the lower travel cost. Furthermore, the sub-population using the faster mode $x$ can potentially access more jobs at further employment centers (say $2$ and $3$) than the sub-population using the slower mode. This puts the sub-population using the faster (lower travel cost) mode $x$ at an accessibility advantage. If that sub-population is also within a population center that is low-population (relative to the region) and central located to employment centers (relative to the region), then this sub-population using the faster mode $x$ is the most advantaged of all sub-populations using any mode or located at any population center.

The illustrative example described is represented in Figure \ref{fig:Fig1} and was inspired by the single-mode example used in @shen1998 and reconfigured in @soukhovIntroducingSpatialAvailability2023. Population center $A$ can be seen as a Suburban population center; it is closest to its own relatively large employment center at $A$, close to the Urban area's equally large employment center $B$, and has a population that is smaller than the Urban center $2$ and larger than the rural center $3$. Consider the number of jobs to be finite in the region. From the perspective of accessibility, the sub-population that is in a center that is the least populated, is the most centrally located to largest amount of jobs, is located away from densely populated centers, and uses the lowest travel cost mode $x$ can potentially access the most job opportunities. From the opposite perspective, sub-populations using the slower mode $z$ and especially those located in population centers that are not centrally located to opportunities and/or close to highly dense population centers, are at an access disadvantage. The number of job opportunities are finite, and the sub-populations using faster modes and/or centrally located take a larger proportion of opportunities, leaving fewer for other populations. 
...
```{r seeing-how-similar-calc-tt-is-to-survey}
...
## Calculating hansen-type accessibility (for comparision)
```{r, echo=FALSE, warning=FALSE, message=FALSE}
OD_Si <- OD_2 %>%
  group_by(VORIZT1259, mode) %>%
...
sum(OD_SA_zn208$SA_ij, na.rm = T)
sum(jobs$jobs)

sum(OD_SA$tt)
sum(OD_SA_zn208$tt)

sum(OD_SA$total_ttworkers)
sum(OD_SA_zn208$total_ttworkers)
```
Spatial availability by mode, we can see that car mode captures ~43% of the spatial availability but only accounts for ~30% of the tt_ijm !!:
```{r}
OD_SA %>%
  group_by(mode) %>%
  summarize(V_m = sum(SA_ij),
            V_m_perc = sum(SA_ij)/847574,
            tt_m = sum(tt)/435061.9,
            ttworkers = sum(total_ttworkers)/440728388)
...
OD_SA_zn208 %>%
  ...
  summarize(V_m = sum(SA_ij),
            V_m_perc = sum(SA_ij)/847574,
            tt_m = sum(tt)/185686.1,
            ttworkers = sum(total_ttworkers)/1211991264)
```

Okay, let's preform a few more checks and then aggregate spatial availability by origin (for both zn1259 and zn208(less 'NAs')) and by origin,mode.
...
Plot V (spatial availability) by origin (first zn1259 then zn208):
```{r}
SA_i_V_zn208_plot <- ggplot() +
  ...
  scale_fill_distiller(palette = "YlGn", #legend scale bar
                       ...
                       na.value = "grey90",
                       direction = 1,
                       label = c("14.7 (min.)", "6248 (1st qu.)", " ", "17768 (3rd qu.)\n14613 (mean)", "51655 (max.)"), 
                       breaks = c(0, quantile(SA_i_zn208$V_i)[2], mean(SA_i_zn208$V_i), quantile(SA_i_zn208$V_i)[4], max(SA_i_zn208$V_i)),
                       limits = c(0, max(SA_i_zn208$V_i)))+
...
annotation_north_arrow(location = "tl", # north arrow for both the main plot and inset
  ...
  annotation_scale(bar_cols = c("dimgrey", "white"), # scale bar for both the main plot and inset
                   height = unit(0.15, "cm")) +
  theme_void() +
  theme(legend.key.width = unit(0.2, 'cm')) # positing legend and clipping out white space
...
SA_i_V_plot <- ggplot() +
  ...
  scale_fill_distiller(palette = "YlGn", #legend scale bar
                       ...
                       na.value = "grey90",
                       direction = 1,
                       label = c("0.9 (min.)", "697 (1st qu.)", "1842 (mean)", "2520 (3rd qu.)", "7463 (max.)"), 
                       breaks = c(0, quantile(SA_i$V_i)[2], mean(SA_i$V_i), quantile(SA_i$V_i)[4], max(SA_i$V_i)),
                       limits = c(0, max(SA_i$V_i)))+
  geom_sf(data = Madrid_muni_border, # border for Madrid muni
          colour=alpha("black",1), 
          size = 0.5, 
          fill=NA, 
  ...
  annotation_north_arrow(location = "tl", # north arrow for both the main plot and inset
                         height = unit(0.7, "cm"), 
                         width = unit(0.7, "cm"),
                         style = north_arrow_orienteering(line_col = "dimgrey", 
                                                          fill = c("white","dimgrey"))) +
  annotation_scale(bar_cols = c("dimgrey", "white"), # scale bar for both the main plot and inset
                   height = unit(0.15, "cm")) +
  theme_void() +
...
```{r}
#mapping population by mode!
ggplot() +
  geom_sf(data = SA_im_zn208,
          aes(fill = population_m),
          color = NA) +
  scale_fill_distiller(palette = "Reds", #legend scale bar
                       na.value = "grey90",
                       direction = 1)+
  geom_sf(data = Madrid_muni_border, # border for Madrid muni
          colour=alpha("black",1), 
  ...
  facet_wrap(~mode, nrow = 2)
ggsave("Presentations/im_populations_zn208_plot.png")
```


```{r}
SA_i_V_zn208_plot
ggsave("Presentations/SA_i_V_zn208_plot.png")
```

```{r}
SA_i_V_plot
ggsave("Presentations/SA_i_V_zn1259_plot.png")
```

Plot v (spatial availability per capita) by origin (first zn1259 then zn208):
```{r}
SA_i_v_plot <- ggplot() +
  geom_sf(data = SA_i,
          aes(fill = v_i),
...
```{r}
SA_i_v_zn208_plot
ggsave("Presentations/SA_i_vv_zn208_plot.png")
```
```{r}
SA_i_v_plot
ggsave("Presentations/SA_i_vv_zn1259_plot.png")
```

Plot V (spatial availability) by origin-mode (first zn1259 then zn208). And for comparision, plot hansen-type accessibility by origin-mode (first zn1259 then zn208):
```{r}
SA_im_V_plot <- ggplot() +
  geom_sf(data = SA_im,
          aes(fill = V_im),
          color = NA) +
  scale_fill_distiller(palette = "YlGn", #legend scale bar
                       name = expression(V["im"]),
                       na.value = "grey90",
                       direction = 1)+
  geom_sf(data = Madrid_muni_border, # border for Madrid munim
          colour=alpha("black",1), 
          size = 0.5, 
          fill=NA, 
          show.legend = "polygon") +
...
#zn208
#the accessibility plot (Hansen-type)
...
```{r}
SA_im_V_plot
ggsave("Presentations/SA_im_V_zn1259_plot.png")
```
```{r}
S_im_plot_zn208_plot
ggsave("Presentations/Hansen_im_V_zn208_plot.png")
```

```{r}
SA_im_V_zn208_plot
ggsave("Presentations/SA_im_V_zn208_plot.png")
```
...
```{r}
SA_im_v_plot
ggsave("Presentations/SA_im_vv_zn1259_plot.png")
```
```{r}
SA_im_v_zn208_plot
ggsave("Presentations/SA_im_vv_zn208_plot.png")
```
