...
column_numbers: 4
logoright_name: Presentation-images/acknowledgements-combo.png
logoleft_name: Presentation-images/soukhova-LinkTree.png
sectitle_textsize: "60pt"
sectitle_borderwidth: "1.5mm"
title_textsize: "110pt"
author_textsize: "80pt"
...
quick_html(ht, file = "Presentation-images/synth_ex_table.html", open = FALSE)
# higher zoom increases resolution and image size
webshot::webshot(url = "Presentation-images/synth_ex_table.html", file = "Presentation-images/synth_ex_table.png",
...
toy_example_plot <- ggplot() + 
  ...
  geom_text(data = od, 
            ...
            size=2.5) +
  # place the table
    annotation_custom(rasterGrob(png::readPNG(source="Presentation-images/synth_ex_table.png")),
                      xmin= 5.5, xmax=Inf, 
                      ymin=6, ymax=Inf
                      )+
  # Theme the plot
...
The most common form of accessibility measure is based on the gravity model, and recent research has focused on the way opportunities are counted (summed). In these calculations, the sums are not constrained, meaning the same opportunity can enter the sum for different origins. This unconstrained counting is a pitfall in conventional methods as it leads to values of accessibility that are difficult to interpret, especially when comparing results between multiple modes.
...
The calculated Hansen (**S**), Shen-type (**a**) and Spatial Availability (**V**) assuming an impedance function of $f(c_{ij})^{\text{x}} = f(c_{ij})^{\text{z}} =exp(-0.1 \cdot c_{ij}^m)$ is shown in the table. 

![](Presentation-images/toy-example.png){#id .class width=100%}
...
# Empirical example: quantifying multimodal access in Madrid
...
![](Presentation-images/Fig2.png){#id .class width=100%}

![](Presentation-images/Fig5.png){#id .class width=95%}
...
![](Presentation-images/Fig6.png){#id .class width=100%}
...
![](Presentation-images/Fig7.png){#id .class width=95%}
