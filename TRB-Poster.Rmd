...
![_Figure 1: Calculated Hansen $S$, Shen-type $a$ and Spatial Availability $V$ assuming an impedance function of $f(c_{ij})^{\text{x}} = f(c_{ij})^{\text{z}} =exp(-0.1 \cdot c_{ij}^m)$ for the multimodal synthetic population._](Presentation-images/toy-example.png){#id .class width=100%}
...
![_Figure 4: Proportion of population by mode and spatial availability of jobs by mode ($V_i^m$) aggregated for the city of Madrid (All), the area within the M-30 highway, and within the Centro._](Presentation-images/Fig6.png){#id .class width=100%}
...
![_Figure 5: Distribution of spatially available jobs per capita by mode of transportatin ($v_i^m$)._](Presentation-images/Fig7.png){#id .class width=98%}
