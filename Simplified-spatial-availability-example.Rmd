...
First, the direct home to work trips:
```{r}
...
```{r}
t <- OD_v %>% group_by(ID_HOGAR,ID_IND) %>% 
...
chained2 <- (ifelse(t$chained[-1]==0 |t$chained==0, 0, NA)) %>% as.data.frame()

t <- cbind(t,chained2) %>% dplyr::rename("chained2" = ".")

t<-t %>% filter(chained2==0) # these are all the CHAINED trips - only considering 1 location before work.  
t
```

Now... let's only include chained trips with the same mode! 
```{r}
t <- t %>% group_by(ID_HOGAR, ID_IND) %>% mutate(trip = ifelse(((ID_VIAJE - lag(ID_VIAJE)) == 1)
                                                               & ((MODO_PRIORITARIO - lag(MODO_PRIORITARIO))== 0), 
                                                               MODO_PRIORITARIO, 0))

t %>% filter(trip == 0) %>% nrow() / t %>% filter(chained == 0) %>% nrow()
t %>% filter(trip != 0 & !is.na(trip)) %>% nrow() / t %>% filter(chained == 0) %>% nrow()
```
so ~27% of chained trips are not made using the same mode. THe rest of them are (`r t %>% filter(trip != 0 & !is.na(trip)) %>% nrow() ` trips are chained (1 intermediate destination) and use the same mode).
...
temp <- read_excel("data-raw/ZonificacionZT1259 complete.xlsx") %>% dplyr::select(ZT1259, Municipality)
zn_t <- zn %>% merge(temp, by = c("ZT1259"), all.x=TRUE)
```

```{r}
Otrips_zona_c <- OD_v_HW_chained %>% group_by(VORIZT1259)  %>% summarise(Otrips = (n())) # we group by the origin ZT1259 ID and then we summarize the number of rows that have the same ZT using the summarize() function and the n() function to create a new 'Otrips' column.

zn_c <- zn %>% merge(Otrips_zona_c, by.x = "ZT1259", by.y="VORIZT1259", all.x = TRUE)
...
Otrips_zona <- OD_v_HW %>% group_by(VORIZT1259)  %>% summarise(Otrips = (n())) # we group by the origin ZT1259 ID and then we summarize the number of rows that have the same ZT using the summarize() function and the n() function to create a new 'Otrips' column.

zn_noc <- zn %>% merge(Otrips_zona, by.x = "ZT1259", by.y="VORIZT1259", all.x = TRUE)
...
so as we can see... the number of origin ZNs with NO origin trips only goes down by 2.. (from 344 to 342). This is not a lot. 

Next step... let's try to see what is happening in these NA zones. What sort of origin trips are being generated? 
```{r}
...
lnorm_ <- fitdistrplus::fitdist(data=walk_HW_alltt$TIEMPO_VIAJES, #%>% unlist() %>% as.numeric(),
                                "lnorm", 
                                method="mle",
                                #Notice the use of the weights,
...
What do the impedance curves tell us about the travel behavior by different modes? Check the histograms:
```{r}
...
OD_v_HW <- OD_v_HW  %>% 
  mutate(mode = case_when(MODO_PRIORITARIO <= 9 ~ "transit",
                          MODO_PRIORITARIO <= 19 & MODO_PRIORITARIO >= 10 ~ "car_motor",
...
NOW WE NEED: the travel times from the routing algorithm between origin-destination pairs. Anastasia did not use the routing, it may not be needed; instead she used the average travel time by mode for all trips 
