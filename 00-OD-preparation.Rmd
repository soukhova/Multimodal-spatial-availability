...
Importing a list of all the municipalities within the Madrid region and adding them to the zone shape file:
```{r}
t <- read_excel("data-raw/ZonificacionZT1259 complete.xlsx") %>% select(ZT1259, Municipality)

zn <- zn %>% merge(t, by = c("ZT1259"), all.x=TRUE)
```
...
ggplot() +
  ...
  scale_fill_distiller(palette = "Spectral")
```
How many trips come into the LEZ from outside of the LEZ and how many of those are car trips? Just curious. 
...
```{r}
OD_v_Oc_Dt_noDesLEZ <- OD_v_Oc_Dt %>% filter(!(VDESZT1259 %in% LEZ_IDS) | (VORIZT1259 %in% LEZ_IDS))
OD_v_Oc_Dt_DesLEZ <- OD_v_Oc_Dt %>% filter((VDESZT1259 %in% LEZ_IDS) & !(VORIZT1259 %in% LEZ_IDS))


count(OD_v_Oc_Dt_DesLEZ)/count(OD_v_Oc_Dt)*100
count(OD_v_Oc_Dt_noDesLEZ) + count(OD_v_Oc_Dt_DesLEZ) ==  count(OD_v_Oc_Dt)
```
we can see that trips into LEZ are a small proportion - only 5.84% of total home-to-work trips in the madrid region have a destination in the LEZ (and do not originate in the LEZ). Now... how many of these are car trips?

```{r}
sum(OD_v_Oc_Dt_DesLEZ$bycar)/count(OD_v_Oc_Dt_DesLEZ)
```
only 19% of these LEZ trips are made by car! This may not have a huge GHG emission savings... but perhaps it has a big impact on equity? 

Let's plot where these car LEZ trips are coming from:
```{r}
D2trips_zona <- OD_v_Oc_Dt_DesLEZ %>% group_by(VORIZT1259)  %>% summarise(Dtrips = (n()),
                                                                              bycar = sum(bycar),
                                                                              nocar = sum(nocar))

zn_t <- zn %>% merge(D2trips_zona, by.x = "ZT1259", by.y="VORIZT1259")
```
...
ggplot() +
  geom_sf(data = zn_t,
          ...
          color  = NA) +
  scale_fill_distiller(palette = "Spectral") +
  labs(title = "% work trips by origin (excluding those that originate in LEZ) made by car ")
