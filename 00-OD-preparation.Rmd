...
```{r}
zn <- zn %>% merge(zn_city %>% st_drop_geometry(), by = c("ZT1259"), all.x=TRUE)
```
...
ggplot() +
  geom_sf(data = zn, aes(fill = as.factor(city)), col = NA) +
  scale_fill_manual(values = c("red", "grey"), name= "Cluster Group")
```
```{r}
#plot to double check that centroids are on zones
ggplot() +
  geom_sf(data = zn_city,) 
```
